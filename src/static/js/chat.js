...
let admin_id = 0;
admin_id = $("#admin-id").text() ? JSON.parse($("#admin-id").text()) : 0;
let socket = ''
// socket = new WebSocket(`ws://${location.host}/${admin_id}`);
...
function display_contact (_) {
    ...
    let el = $(`.chat-users .chat-user#${_.id}`);
...
function write_message (id, user_image, branch_image, content, date, type, link, name, size, who_send, current=false) {
    ...
    if ( who_send == admin_id ) { cls = "sender"; image = branch_image; }
