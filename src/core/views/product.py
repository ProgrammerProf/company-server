...
def index(request):
    ...
    data['products'] = list(reversed(product.objects.filter(role=1)))
...
def add(request):
    ...
    if request.method == "POST":
        ...
        product(
            ...
            category_id=category_id, active=active, date=get_date(), company=company,
            phone=phone, number=number, overview=overview, included=included,
            expire_date=expire_date, allow_bookings=allow_bookings, keys=keys, brochure_url=brochure_url,
            study_url=study_url
        ).save()
...
def edit(request, id):
    data = start_data(request)
    if not data['user'].products: return redirect('/')
    if not product.objects.filter(id=id, role=1).exists(): return redirect("/products")
    data['product'] = product.objects.get(id=id, role=1)
    data['categories'] = category.objects.filter(active=True, products=True)
